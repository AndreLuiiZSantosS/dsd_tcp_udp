...
class AgarGame:
    def __init__(self, root, player_name, server_ip):
        ...
        self.my_color = random.choice(
            ['#FF0000', '#0000FF', '#00FF00', '#FFA500', '#8A2BE2']
        )
    ...
    def connect_sockets(self):
        try:
            ...
            # UDP (BIND OBRIGATÓRIO PRA LAN / WIFI)
            self.udp = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            self.udp.bind(("", 0))  # <<< ESSENCIAL
            self.udp.sendto(b"INIT", (self.server_ip, UDP_PORT))
    ...
    def check_collisions(self):
        ...
        if to_remove:
            ...
            if to_remove in self.foods:
                del self.foods[to_remove]
            self.grow(2)
    ...
    def game_loop(self):
        ...
        try:
            self.udp.sendto(msg.encode(), (self.server_ip, UDP_PORT))
        except:
            pass
        ...
        for pname, pdata in self.other_players.items():
            ...
            self.canvas.create_oval(px - ps, py - ps, px + ps, py + ps, fill=pc)
            self.canvas.create_text(px, py - ps - 15, text=pname, fill="white")
    ...
    def listen_udp(self):
        while not self.game_over:
            try:
                ...
                if len(parts) == 5:
                    pname, px, py, ps, pc = parts
                    if pname != self.name:
                        self.other_players[pname] = {
                            'x': int(px),
                            'y': int(py),
                            's': int(ps),
                            'c': pc
    ...
    def listen_tcp(self):
        while not self.game_over:
            try:
                ...
                elif msg.startswith("RMV_FOOD:"):
                    fid = msg.split(":")[1]
                    if fid in self.foods:
                        del self.foods[fid]

                elif msg.startswith("NEW_FOOD:"):
                    fid, x, y, c = msg.split(":")[1].split(",")
                    self.foods[fid] = {'x': int(x), 'y': int(y), 'c': c}

                elif msg.startswith("DIE:"):
                    victim = msg.split(":")[1]
                    if victim == self.name:
                        self.add_log(">>> VOCÊ FOI DEVORADO!")
                        self.respawn()
...
class MainMenu:
    def __init__(self, root):
        ...
        tk.Label(root, text="Seu Nickname:", bg="#111", fg="white").pack()
        self.entry_name = tk.Entry(root)
        self.entry_name.pack(pady=5)

        tk.Label(root, text="IP do Host:", bg="#111", fg="white").pack()
        self.entry_ip = tk.Entry(root)
        self.entry_ip.insert(0, "10.25.1.231")  # <<< IP DA LAN
